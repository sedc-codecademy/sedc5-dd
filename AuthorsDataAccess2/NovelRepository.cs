...
namespace AuthorsDataAccess2
{
    public class NovelRepository : INovelRepository
    {
        ...
        {
            int x = 0;
            if (GetNovelsByAuthor(authorId, ref x).Any(a => a.Title == title))
            {
        ...
        public IEnumerable<Novel> GetAllNovels(ref int counter)
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            novels.Add(LoadNovelFromReader(dr));
                        }
                    }
                }
            }
        ...
        public IEnumerable<Novel> GetNovelsByAuthor(int authorId, ref int counter)
        {
            var novels = new List<Novel>();
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
        ...
        private Novel LoadNovelFromReader(SqlDataReader dr)
        {
